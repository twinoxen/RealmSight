...
export default function HUD({ sceneRef }: HUDProps) {
  ...
  return (
    <div
      style={{ position: 'absolute', inset: 0, pointerEvents: 'all', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', zIndex: 3 }}
      onTouchStart={onTap}
      onClick={onTap}
    >
      {/* Top bar */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '16px', paddingTop: 'max(16px, env(safe-area-inset-top))' }}>
        {modeLabel && (
          <span style={{ pointerEvents: 'none', background: 'rgba(0,0,0,0.55)', padding: '4px 10px', borderRadius: 20, fontSize: 12, color: '#fff' }}>
            {modeLabel}
          </span>
        )}
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          {isActive && (
            <button
              style={{ pointerEvents: 'all', background: 'rgba(0,0,0,0.5)', border: 'none', borderRadius: 20, padding: '6px 14px', color: '#fff', fontSize: 13 }}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', padding: '24px 16px', paddingBottom: 'max(24px, env(safe-area-inset-bottom))' }}>
        ...
        <button
          style={{ pointerEvents: 'all', background: isActive ? 'rgba(239,68,68,0.85)' : 'rgba(99,102,241,0.9)', border: 'none', borderRadius: 32, padding: '14px 28px', color: '#fff', fontSize: 16, fontWeight: 600 }}
          onClick={e => { e.stopPropagation(); isActive ? stop() : start() }}
        >
