...
export class ARSession {
  ...
  placeShapeAtScreen(x: number, y: number) {
    ...
    if (!hit) target.copy(raycaster.ray.origin).addScaledVector(raycaster.ray.direction, 1.2)
    this.spawnShape(target, new THREE.Quaternion())
  }

  /** Place a glyph-specific shape at a normalized screen position (from vision pipeline) */
  ...
  private spawnShape(pos: THREE.Vector3, quat: THREE.Quaternion) {
    ...
    const geo = this.placedShapes.length % 2 === 0
  ...
  private spawnGlyphShape(pos: THREE.Vector3, glyphLabel: string) {
    const colorMap: Record<string, number> = {
      house: 0x6366f1, castle: 0x8b5cf6, bridge: 0x3b82f6,
      road: 0x6b7280, river: 0x06b6d4, tree: 0x10b981,
      mountain: 0x64748b, temple: 0xf59e0b,
    }
    const color = colorMap[glyphLabel] ?? 0xffffff
    let geo: THREE.BufferGeometry
    switch (glyphLabel) {
      case 'house':    geo = new THREE.ConeGeometry(0.07, 0.12, 4); break
      case 'castle':   geo = new THREE.BoxGeometry(0.1, 0.15, 0.1); break
      case 'bridge':   geo = new THREE.TorusGeometry(0.06, 0.02, 8, 16, Math.PI); break
      case 'river':    geo = new THREE.CylinderGeometry(0.02, 0.02, 0.2, 8); break
      case 'tree':     geo = new THREE.ConeGeometry(0.06, 0.14, 6); break
      case 'mountain': geo = new THREE.ConeGeometry(0.08, 0.16, 3); break
      case 'temple':   geo = new THREE.BoxGeometry(0.08, 0.14, 0.08); break
      default:         geo = new THREE.SphereGeometry(0.06, 16, 16)
    }
    const mat = new THREE.MeshStandardMaterial({ color, roughness: 0.4, metalness: 0.2 })
    const mesh = new THREE.Mesh(geo, mat)
    mesh.position.copy(pos)
